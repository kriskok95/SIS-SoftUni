...
namespace SIS.HTTP.Requests
{
    public class HttpRequest : IHttpRequest
    {
        ...
        private void ParseCookies()
        {
            ...
            var rawCookie = this.Headers.GetHeader("Cookie").Value;
            var cookiesArr = rawCookie.Split("; ", StringSplitOptions.RemoveEmptyEntries);

            foreach (var cookiesRaw in cookiesArr)
            {
                var cookieKvp = cookiesRaw.Split('=', 2);
                ...
                string cookieName = cookieKvp[0];
                string cookieValue = cookieKvp[1];
